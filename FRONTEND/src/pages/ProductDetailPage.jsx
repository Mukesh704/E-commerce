...
import { getProductById, submitReview, getReviews } from '../services/api';
...
const ProductDetailPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      <h1 className="text-2xl sm:text-4xl font-bold text-gray-900 mb-6 sm:mb-8">{product.name}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          {/* Main Image */}
          <div className="flex-1 rounded bg-white overflow-hidden h-64 sm:h-[550px]">
            <img
        ...
        <div className="space-y-4">
          <h2 className="text-xl sm:text-2xl font-bold text-white bg-gray-800 px-4 py-2 rounded">{product.name}</h2>
          ...
          <div className="flex flex-wrap gap-2">
            {(product.categories || []).map((cat, i) => (
              <span key={i} className="flex items-center gap-1 bg-gray-200 text-xs sm:text-sm px-2 py-1 rounded">
          ...
          <p className="text-2xl sm:text-3xl font-bold text-green-700">₹{product.price}</p>
          <p className={`text-sm sm:text-md font-semibold ${product.stock > 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 mt-4">
            <Button onClick={handleAddToCart} className="flex-1 sm:flex-none bg-black text-white hover:bg-gray-300 hover:text-black">Add to Cart</Button>
            <Button onClick={handleBuyNow} className="flex-1 sm:flex-none bg-black text-white hover:bg-gray-300 hover:text-black">Buy Now</Button>
            <Button
              onClick={toggleWishlist}
              className="flex-1 sm:flex-none group flex items-center justify-center gap-2 border border-black text-white hover:bg-gray-300 hover:text-black"
            >
              {isFavourite ? (
      ...
      <section className="mt-12">
        <h2 className="text-xl sm:text-2xl font-bold mb-4 text-black">Customer Reviews</h2>
        ...
        <form onSubmit={handleReviewSubmit} className="mt-6 space-y-4">
          ...
          <Button type="submit" className="bg-black text-white hover:bg-gray-300 hover:text-black">
            Submit Review
          </Button>
        </form>
      </section>
